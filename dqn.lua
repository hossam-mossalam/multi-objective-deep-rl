...
return function (ols_parameters)
  ...
  for e = 1, opt.nepisodes do
    ...
    if e == 1 or e % opt.step == 0 then
      ...
      log.infof('e=%d, train_q=%.3f, train_q_avg=%.3f, ' ..
          'last_train_r=%.3f;%.3f, train_r_avg=%.3f;%.3f, test_r=%.3f,%.3f, ' ..
          't/e=%.2f sec, t=%d min., test_step_avg=%.3f, ' ..
          'train_episodes_reached_goal=%d, test_episodes_reached_goal=%d, ' ..
          'current_weight = < %.4f, %.4f >, test_step_avg=%.2f',
          e, train_q, train_q_avg, train_r[1][1], train_r[1][2],
          train_r_avg[1][1], train_r_avg[1][2], test_r[1], test_r[2],
          sys.clock() - time, torch.toc(beginning_time) / 60,
          test_step_all:narrow(1,1,e):mean(), train_episodes_reached_goal,
          test_episodes_reached_goal, weight[1]:squeeze(), weight[2]:squeeze(),
          test_step_all:narrow(1,1,e):sum()/test_episodes_reached_goal)
